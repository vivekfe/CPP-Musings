...
If your compiler produces a NaN, it has the unusual property that it is not equal to any value, including itself. For example, if a is NaN, then a == a is false. so use isnan() from  standard library.  In fact, if a is NaN, then a will be neither less than, equal to, nor greater than any value including itself.
...
In another way, = delete means that the compiler will not generate those constructors when declared and this is only allowed on copy constructor and assignment operator. 
...
This is called uniform initialization, and it uses curly braces {} instead of parentheses (). This was introduced in C++11 and it's a more modern way of initializing variables.

Using curly braces {} for initializing variables has some advantages over using parentheses (). One of the main advantages is that it can help prevent narrowing conversions. Narrowing conversions are when a value is assigned to a variable that cannot represent the value, such as trying to assign a double to an int. When using curly braces, the compiler will give an error if there is a narrowing conversion, while with parentheses, the conversion will be allowed. This can help prevent bugs in the code. Additionally, curly braces can be used for initializing non-POD (Plain Old Data) types like std::string, std::vector, etc., where as the parentheses can only be used for POD types like int, float, char, etc.
...
Both forms will create an object of the class and call the default constructor, so the resulting behavior will be the same. However, the first form is more explicit and can be useful in some cases, such as when you want to make it clear that you are calling the default constructor. The second form is more concise and is more commonly used.

It's worth noting that when using new operator to create array of objects, you must use the first form, new ClassName[size]() in order to call the default constructor for each element.

The second form, new ClassName, does indeed call the default constructor, just like the first form, new ClassName().

In C++, when a class does not have a user-defined constructor, the compiler will automatically provide a default constructor. This default constructor is called when an object of the class is created using the new operator, whether you explicitly call it with empty parentheses or not. In both forms, new ClassName() and new ClassName, the default constructor is called, and the resulting behavior is the same.
